...
type HelmInstallConfig struct {
	...
	// HelmLoggerFunc is called for Helm operation logging. Defaults to no-op.
...
func (c *HelmInstallConfig) Validate() error {
	...
	if c.HelmLoggerFunc == nil {
		c.HelmLoggerFunc = func(format string, v ...interface{}) {}
	}
	return nil
}
...
func InstallHelmChart(config *HelmInstallConfig) (*release.Release, error) {
	...
	chartPath, err := installClient.ChartPathOptions.LocateChart(config.ChartRef, cli.New())
	if err != nil {
