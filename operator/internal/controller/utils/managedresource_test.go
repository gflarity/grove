...
func TestIsManagedByGrove(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		if tc.labels == nil {
			testName = "nil_labels"
		} else if val, ok := tc.labels[apicommon.LabelManagedByKey]; !ok {
			testName = "missing_managed_by_label"
		} else if val != apicommon.LabelManagedByValue {
...
func TestIsManagedPodClique(t *testing.T) {
	...
	for i, tc := range testCases {
		...
		} else if tc.obj.GetLabels() == nil || tc.obj.GetLabels()[apicommon.LabelManagedByKey] != apicommon.LabelManagedByValue {
