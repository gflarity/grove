...
func TestMapPGSToPCSG(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// Test mapping with valid PGS during rolling update with multiple scaling group configs
			name:                 "valid_pgs_during_rolling_update_with_multiple_configs",
			inputObj:             createTestPGSWithScalingGroupsAndRollingUpdate("test-pgs", "test-ns", 2, []string{"sg1", "sg2"}, 0),
			expectedRequestCount: 2, // Only current replica (0) * 2 scaling groups
			expectedRequests: []reconcile.Request{
		...
		{
			// Test mapping with valid PGS during rolling update with single scaling group config
			name:                 "valid_pgs_during_rolling_update_with_single_config",
			inputObj:             createTestPGSWithScalingGroupsAndRollingUpdate("test-pgs", "test-ns", 1, []string{"sg1"}, 0),
		...
		{
			// Test mapping with PGS not during rolling update (no CurrentlyUpdating status)
			name:                 "valid_pgs_not_during_rolling_update",
			inputObj:             createTestPGSWithScalingGroups("test-pgs", "test-ns", 2, []string{"sg1", "sg2"}),
			expectedRequestCount: 0, // No rolling update in progress
			expectedRequests:     nil,
		},
		{
			// Test mapping with PGS during rolling update with no scaling group configs
			name:                 "valid_pgs_during_rolling_update_with_no_configs",
			inputObj:             createTestPGSWithScalingGroupsAndRollingUpdate("test-pgs", "test-ns", 1, []string{}, 0),
...
func TestPodGangSetPredicate(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			// Test that update events are allowed for Grove-managed PodGangSets with rolling update changes
			name: "update_event_grove_managed",
			testFunc: func(t *testing.T) {
				// Create old PGS with no rolling update in progress
				oldObj := createTestPGS("test-pgs", "test-ns")
				addGroveLabels(oldObj)
				oldObj.Status.RollingUpdateProgress = &grovecorev1alpha1.PodGangSetRollingUpdateProgress{
					CurrentlyUpdating: nil, // No current update
				}

				// Create new PGS with rolling update started
				newObj := createTestPGS("test-pgs", "test-ns")
				addGroveLabels(newObj)
				newObj.Status.RollingUpdateProgress = &grovecorev1alpha1.PodGangSetRollingUpdateProgress{
				...
				event := event.UpdateEvent{ObjectOld: oldObj, ObjectNew: newObj}
				result := predicate.Update(event)
				assert.True(t, result, "Should allow update events for Grove-managed PodGangSet with rolling update changes")
			},
		},
		{
			// Test that update events are rejected when there are no rolling update changes
			name: "update_event_grove_managed_no_rolling_update_changes",
			testFunc: func(t *testing.T) {
				oldObj := createTestPGS("test-pgs", "test-ns")
				addGroveLabels(oldObj)
				...
				event := event.UpdateEvent{ObjectOld: oldObj, ObjectNew: newObj}
				result := predicate.Update(event)
				assert.False(t, result, "Should reject update events for Grove-managed PodGangSet without rolling update changes")
			},
		},
		{
...
// createTestPGSWithScalingGroupsAndRollingUpdate creates a test PodGangSet with scaling group configurations and rolling update status.
func createTestPGSWithScalingGroupsAndRollingUpdate(name, namespace string, replicas int32, scalingGroupNames []string, currentlyUpdatingReplicaIndex int32) *grovecorev1alpha1.PodGangSet {
	pgs := createTestPGSWithScalingGroups(name, namespace, replicas, scalingGroupNames)

	// Set up rolling update status
	pgs.Status.RollingUpdateProgress = &grovecorev1alpha1.PodGangSetRollingUpdateProgress{
		CurrentlyUpdating: &grovecorev1alpha1.PodGangSetReplicaRollingUpdateProgress{
			ReplicaIndex: currentlyUpdatingReplicaIndex,
		},
	}
...
func createTestPCLQWithPCSGLabel(name, namespace, pcsgName string) *grovecorev1alpha1.PodClique {
	...
	pclq.Labels[apicommon.LabelPodCliqueScalingGroup] = pcsgName
	return pclq
}
