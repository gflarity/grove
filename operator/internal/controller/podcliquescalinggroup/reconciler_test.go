			...
			existingObjects: []client.Object{
				...
				&grovecorev1alpha1.PodCliqueScalingGroup{
					ObjectMeta: metav1.ObjectMeta{
						...
						Namespace:  "default",
						Finalizers: []string{grovecorev1alpha1.FinalizerPodCliqueScalingGroup},
...
func TestReconcileStatusFlow(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := log.IntoContext(context.Background(), logr.Discard())
			result := reconciler.reconcileStatus(ctx, logr.Discard(), tt.pcsg)
...
func TestReconcilerEnsureFinalizer(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedResult:           ctrlcommon.ReconcileWithErrors("error adding finalizer", fmt.Errorf("failed to add finalizer: %s to PodCliqueScalingGroup: %v: %w", grovecorev1alpha1.FinalizerPodCliqueScalingGroup, types.NamespacedName{Name: "test-pcsg", Namespace: "default"}, errors.New("patch operation failed"))),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedFinalizerPresent {
				assert.Contains(t, tt.pcsg.Finalizers, grovecorev1alpha1.FinalizerPodCliqueScalingGroup, "Expected finalizer to be present")
			}
...
func TestReconcilerRemoveFinalizer(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedResult:           ctrlcommon.ReconcileWithErrors("error removing finalizer", fmt.Errorf("failed to remove finalizer: %s from PodCliqueScalingGroup: %v: %w", grovecorev1alpha1.FinalizerPodCliqueScalingGroup, types.NamespacedName{Name: "test-pcsg", Namespace: "default"}, errors.New("patch operation failed"))),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedFinalizerRemoved {
				assert.NotContains(t, tt.pcsg.Finalizers, grovecorev1alpha1.FinalizerPodCliqueScalingGroup, "Expected finalizer to be removed")
			} else {
				// If we didn't expect removal, check if the finalizer was originally present
				hadFinalizer := false
				for _, finalizer := range originalFinalizers {
					if finalizer == grovecorev1alpha1.FinalizerPodCliqueScalingGroup {
						hadFinalizer = true
						break
					}
				}
				if hadFinalizer {
					...
					assert.Contains(t, tt.pcsg.Finalizers, grovecorev1alpha1.FinalizerPodCliqueScalingGroup, "Expected finalizer to remain")
				}
			}
		})
	}
...
func createStandardTestPCSG(name, namespace string, withFinalizer bool) *grovecorev1alpha1.PodCliqueScalingGroup {
	...
	if withFinalizer {
		pcsg.Finalizers = []string{grovecorev1alpha1.FinalizerPodCliqueScalingGroup}
	}
