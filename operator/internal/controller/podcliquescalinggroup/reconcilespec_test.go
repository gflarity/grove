...
func createTestPCSG(name, namespace string, withFinalizer bool) *grovecorev1alpha1.PodCliqueScalingGroup {
	...
	if withFinalizer {
		controllerutil.AddFinalizer(pcsg, constants.FinalizerPodCliqueScalingGroup)
	}
...
func TestEnsureFinalizer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			initialHasFinalizer := controllerutil.ContainsFinalizer(tt.pcsg, constants.FinalizerPodCliqueScalingGroup)
			...
			currentHasFinalizer := controllerutil.ContainsFinalizer(tt.pcsg, constants.FinalizerPodCliqueScalingGroup)
