	...
	tests := []struct {
	...
	}{
		{
			...
			setupPodClique: func() *grovecorev1alpha1.PodClique {
				...
				pclq.Labels[common.LabelPartOfKey] = "test-pgs"
				return pclq
			},
			expectedRequests: []reconcile.Request{
	...
	tests := []struct {
	...
	}{
		{
			...
			setupPCSG: func() *grovecorev1alpha1.PodCliqueScalingGroup {
				...
				pcsg.Labels[common.LabelPartOfKey] = "test-pgs"
				return pcsg
			},
			expectedRequests: []reconcile.Request{
				{NamespacedName: types.NamespacedName{Name: "test-pgs", Namespace: "test-namespace"}},
...
// TestHasRollingUpdateStatusChanged validates rolling update status change detection for various scenarios.
func TestHasRollingUpdateStatusChanged(t *testing.T) {
	tests := []struct {
		// Test case name describing the scenario being tested
		name string
		// Old status setup
		oldStatus grovecorev1alpha1.PodCliqueScalingGroupStatus
		// New status setup
		newStatus grovecorev1alpha1.PodCliqueScalingGroupStatus
		// Expected result
		expected bool
	}{
		{
			// Rolling update progress changed should be detected
			name: "rolling update progress changed detected",
			oldStatus: grovecorev1alpha1.PodCliqueScalingGroupStatus{
				RollingUpdateProgress: &grovecorev1alpha1.PodCliqueScalingGroupRollingUpdateProgress{
					UpdateStartedAt: metav1.Time{Time: time.Date(2023, 1, 1, 12, 0, 0, 0, time.UTC)},
				},
			},
			newStatus: grovecorev1alpha1.PodCliqueScalingGroupStatus{
				RollingUpdateProgress: &grovecorev1alpha1.PodCliqueScalingGroupRollingUpdateProgress{
					UpdateStartedAt: metav1.Time{Time: time.Date(2023, 1, 1, 12, 1, 0, 0, time.UTC)},
				},
			},
			expected: true,
		},
		{
			// Rolling update progress added should be detected
			name: "rolling update progress added detected",
			oldStatus: grovecorev1alpha1.PodCliqueScalingGroupStatus{
		...
		{
			// Rolling update progress removed should be detected
			name: "rolling update progress removed detected",
			oldStatus: grovecorev1alpha1.PodCliqueScalingGroupStatus{
				RollingUpdateProgress: &grovecorev1alpha1.PodCliqueScalingGroupRollingUpdateProgress{
					UpdateStartedAt: metav1.Time{Time: time.Date(2023, 1, 1, 12, 0, 0, 0, time.UTC)},
				},
		...
		{
			// No rolling update progress changes should not be detected
			name: "no rolling update progress changes not detected",
			oldStatus: grovecorev1alpha1.PodCliqueScalingGroupStatus{
				RollingUpdateProgress: &grovecorev1alpha1.PodCliqueScalingGroupRollingUpdateProgress{
					UpdateStartedAt: metav1.Time{Time: time.Date(2023, 1, 1, 12, 0, 0, 0, time.UTC)},
				},
		...
		{
			// Both nil should not be detected
			name: "both rolling update progress nil not detected",
			oldStatus: grovecorev1alpha1.PodCliqueScalingGroupStatus{
				RollingUpdateProgress: nil,
			},
			newStatus: grovecorev1alpha1.PodCliqueScalingGroupStatus{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := hasRollingUpdateStatusChanged(&tt.oldStatus, &tt.newStatus)
			assert.Equal(t, tt.expected, result)
		})
	}
}
