...
func TestGetExistingResourceNames(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			existingObjects: []client.Object{
				&rbacv1.RoleBinding{
					ObjectMeta: metav1.ObjectMeta{
						Name:      grovecorev1alpha1.GeneratePodRoleBindingName("test-pgs"),
						Namespace: "default",
			...
			expectedNames: []string{grovecorev1alpha1.GeneratePodRoleBindingName("test-pgs")},
			expectError:   false,
		},
		{
...
func TestGetLabels(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedLabels: map[string]string{
				...
				grovecorev1alpha1.LabelAppNameKey:   "grove.io-pgs-test-pgs",
		...
		},
		{
			...
			expectedLabels: map[string]string{
				grovecorev1alpha1.LabelComponentKey: component.NamePodRoleBinding,
				grovecorev1alpha1.LabelAppNameKey:   "grove.io-pgs-test-pgs-with-colons",
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Contains(t, labels, grovecorev1alpha1.LabelComponentKey)
			assert.Contains(t, labels, grovecorev1alpha1.LabelAppNameKey)
			assert.Equal(t, component.NamePodRoleBinding, labels[grovecorev1alpha1.LabelComponentKey])
		})
	}
}
