...
func TestGetExistingResourceNames(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			existingSecrets: []client.Object{
				&corev1.Secret{
					ObjectMeta: metav1.ObjectMeta{
						Name:      grovecorev1alpha1.GenerateInitContainerSATokenSecretName("test-pgs"),
						Namespace: "default",
			...
			expectedNames: []string{grovecorev1alpha1.GenerateInitContainerSATokenSecretName("test-pgs")},
			expectError:   false,
		},
		{
			...
			if tt.expectError {
				...
				if tt.expectSecretCreated {
					secretKey := client.ObjectKey{
						Name:      grovecorev1alpha1.GenerateInitContainerSATokenSecretName(tt.pgs.Name),
					...
					assert.Equal(t, grovecorev1alpha1.GeneratePodServiceAccountName(tt.pgs.Name),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				...
				secretKey := client.ObjectKey{
					Name:      grovecorev1alpha1.GenerateInitContainerSATokenSecretName(tt.pgsObjMeta.Name),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				...
				// Verify annotations
				expectedSAName := grovecorev1alpha1.GeneratePodServiceAccountName(tt.pgs.Name)
...
func TestGetLabels(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedLabels: map[string]string{
				...
				grovecorev1alpha1.LabelAppNameKey:   "test-secret",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, component.NameServiceAccountTokenSecret, labels[grovecorev1alpha1.LabelComponentKey],
				"Component label should be set correctly")
			assert.Equal(t, tt.secretName, labels[grovecorev1alpha1.LabelAppNameKey],
			...
			assert.Contains(t, labels, grovecorev1alpha1.LabelComponentKey, "Should contain component label")
			assert.Contains(t, labels, grovecorev1alpha1.LabelAppNameKey, "Should contain app name label")
		})
	}
}
...
func TestGetObjectKey(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedKey: client.ObjectKey{
				Name:      grovecorev1alpha1.GenerateInitContainerSATokenSecretName("my-pgs"),
				Namespace: "custom-ns",
			},
		},
	}
