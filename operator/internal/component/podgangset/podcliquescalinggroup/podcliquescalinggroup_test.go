...
// TestDoCreate validates the doCreate method which handles the creation of individual
...
func TestDoCreate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Tests handling of already exists error during creation.
			// Should ignore the error and proceed successfully.
		...
		{
			// Tests error handling when creation fails with non-AlreadyExists error.
			...
			expectError:   true,
			errorContains: "Error creating PodCliqueScalingGroup",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := operator.doCreate(ctx, logger, tt.pgs, tt.pgsReplica, tt.pcsgObjectKey, tt.pcsgConfig)
			...
			if tt.expectError {
				...
				case event := <-eventRecorder.Events:
					assert.Contains(t, event, groveevents.ReasonPodCliqueScalingGroupCreationFailed)
				...
				// Check that success event was recorded (unless it was an AlreadyExists case)
				if !apierrors.IsAlreadyExists(err) {
					select {
					case event := <-eventRecorder.Events:
						assert.Contains(t, event, groveevents.ReasonPodCliqueScalingGroupCreationSuccessful)
						assert.Contains(t, event, "Normal")
					default:
						// For AlreadyExists case, no event is expected
					}
				}
			}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectError {
				...
				select {
				case event := <-eventRecorder.Events:
					assert.Contains(t, event, groveevents.ReasonPodCliqueScalingGroupDeletionFailed)
					assert.Contains(t, event, "Warning")
				default:
					t.Error("Expected warning event to be recorded")
				}
				...
				select {
				case event := <-eventRecorder.Events:
					assert.Contains(t, event, groveevents.ReasonPodCliqueScalingGroupDeletionSuccessful)
					assert.Contains(t, event, "Normal")
				default:
...
func TestBuildResource(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			validateFunc: func(t *testing.T, pcsg *grovecorev1alpha1.PodCliqueScalingGroup) {
				...
				assert.Equal(t, component.NamePodCliqueScalingGroup, pcsg.Labels[grovecorev1alpha1.LabelComponentKey])
				assert.Equal(t, "0", pcsg.Labels[grovecorev1alpha1.LabelPodGangSetReplicaIndex])
			},
		},
		{
			...
			expectError: false,
			validateFunc: func(t *testing.T, pcsg *grovecorev1alpha1.PodCliqueScalingGroup) {
				assert.Equal(t, "2", pcsg.Labels[grovecorev1alpha1.LabelPodGangSetReplicaIndex])
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := operator.buildResource(pcsg, tt.pgs, tt.pgsReplica, tt.pcsgConfig)
...
func TestGetLabels(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedLabels: map[string]string{
				grovecorev1alpha1.LabelAppNameKey:             "test-pgs-0-sg1",
		...
		},
		{
			...
			expectedLabels: map[string]string{
				grovecorev1alpha1.LabelAppNameKey:             "my-pgs-5-scaling-group",
				grovecorev1alpha1.LabelComponentKey:           component.NamePodCliqueScalingGroup,
				grovecorev1alpha1.LabelPodGangSetReplicaIndex: "5",
				grovecorev1alpha1.LabelPartOfKey:              "my-pgs",
				grovecorev1alpha1.LabelManagedByKey:           grovecorev1alpha1.LabelManagedByValue,
			},
		},
		{
			...
			expectedLabels: map[string]string{
				grovecorev1alpha1.LabelAppNameKey:             "very-long-podgangset-name-for-testing-10-very-long-scaling-group-name",
				grovecorev1alpha1.LabelComponentKey:           component.NamePodCliqueScalingGroup,
				grovecorev1alpha1.LabelPodGangSetReplicaIndex: "10",
				grovecorev1alpha1.LabelPartOfKey:              "very-long-podgangset-name-for-testing",
				grovecorev1alpha1.LabelManagedByKey:           grovecorev1alpha1.LabelManagedByValue,
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			labels := getLabels(tt.pgsName, tt.pgsReplica, tt.pclqScalingGroupObjKey)
...
func TestGetPodCliqueScalingGroupSelectorLabels(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedLabels: map[string]string{
				grovecorev1alpha1.LabelPartOfKey:    "minimal-pgs",
				grovecorev1alpha1.LabelComponentKey: component.NamePodCliqueScalingGroup,
				grovecorev1alpha1.LabelManagedByKey: grovecorev1alpha1.LabelManagedByValue,
			},
		},
