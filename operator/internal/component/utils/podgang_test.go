...
func TestGetPodGangSelectorLabels(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedLabels: map[string]string{
				grovecorev1alpha1.LabelManagedByKey: grovecorev1alpha1.LabelManagedByValue,
				grovecorev1alpha1.LabelPartOfKey:    "labeled-pgs",
				grovecorev1alpha1.LabelComponentKey: component.NamePodGang,
			},
		},
		{
		...
		},
		{
			...
			expectedLabels: map[string]string{
				grovecorev1alpha1.LabelManagedByKey: grovecorev1alpha1.LabelManagedByValue,
				grovecorev1alpha1.LabelPartOfKey:    "test-pgs-123_special",
				grovecorev1alpha1.LabelComponentKey: component.NamePodGang,
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Contains(t, labels, grovecorev1alpha1.LabelManagedByKey, "should contain managed-by label")
			assert.Contains(t, labels, grovecorev1alpha1.LabelPartOfKey, "should contain part-of label")
			assert.Contains(t, labels, grovecorev1alpha1.LabelComponentKey, "should contain component label")

			// Verify specific label values
			assert.Equal(t, grovecorev1alpha1.LabelManagedByValue, labels[grovecorev1alpha1.LabelManagedByKey], "managed-by label should have correct value")
			assert.Equal(t, tt.objMeta.Name, labels[grovecorev1alpha1.LabelPartOfKey], "part-of label should match PodGangSet name")
			assert.Equal(t, component.NamePodGang, labels[grovecorev1alpha1.LabelComponentKey], "component label should be 'podgang'")
		})
	}
}
