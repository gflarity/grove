...
func TestSync(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := resource.Sync(context.Background(), testutils.SetupTestLogger(), tt.pclq)
	...
	tests := []struct {
		...
		},
		{
			...
			expectedLabels: map[string]string{
				...
				grovecorev1alpha1.LabelPodClique:    "orphan-pclq",
				grovecorev1alpha1.LabelManagedByKey: grovecorev1alpha1.LabelManagedByValue,
			},
		},
	}
...
func TestGetLabels(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedLabelsContain: map[string]string{
				grovecorev1alpha1.LabelPodClique:              "test-pclq",
				grovecorev1alpha1.LabelPodGangSetReplicaIndex: "2",
				grovecorev1alpha1.LabelPodGang:                "test-podgang",
				grovecorev1alpha1.LabelPartOfKey:              "test-pgs",
				"user-label":                                  "user-value",
				"app":                                         "my-app",
			},
		},
		{
			...
			pgsReplicaIndex: 0,
			expectedLabelsContain: map[string]string{
				grovecorev1alpha1.LabelPodClique:              "minimal-pclq",
				grovecorev1alpha1.LabelPodGangSetReplicaIndex: "0",
				grovecorev1alpha1.LabelPodGang:                "minimal-podgang",
	...
	}{
		{
			...
			pclq: &grovecorev1alpha1.PodClique{
				ObjectMeta: metav1.ObjectMeta{
					...
					Labels: map[string]string{
						"user-label":                        "user-value",
						grovecorev1alpha1.LabelPartOfKey:    "inference",
						grovecorev1alpha1.LabelManagedByKey: grovecorev1alpha1.LabelManagedByValue,
					},
			...
			validatePod: func(t *testing.T, pod *corev1.Pod) {
				...
				assert.Equal(t, "inference-0-prefill", pod.Labels[grovecorev1alpha1.LabelPodClique])
				assert.Equal(t, "0", pod.Labels[grovecorev1alpha1.LabelPodGangSetReplicaIndex])
				assert.Equal(t, "test-podgang", pod.Labels[grovecorev1alpha1.LabelPodGang])
				...
				assert.Equal(t, "inference", envVarMap[grovecorev1alpha1.EnvVarPGSName])
				assert.Equal(t, "0", envVarMap[grovecorev1alpha1.EnvVarPGSIndex])
				assert.Equal(t, "inference-0-prefill", envVarMap[grovecorev1alpha1.EnvVarPCLQName])
				assert.Equal(t, "2", envVarMap[grovecorev1alpha1.EnvVarPodIndex])
			},
		},
		{
