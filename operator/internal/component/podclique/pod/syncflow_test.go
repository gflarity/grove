	...
	tests := []struct {
		...
		// Successfully prepares sync context when all dependencies exist
		{
			...
			setupObjects: []client.Object{
				...
				createTestPodWithName("test-pod-0", "test-pgs", "test-pclq"),
				createTestPodWithName("test-pod-1", "test-pgs", "test-pclq"),
			},
			expectError:                   false,
		...
		// Returns error when owner PodGangSet does not exist
		{
			...
			pclq: &grovecorev1alpha1.PodClique{
				ObjectMeta: metav1.ObjectMeta{
					...
					Labels: map[string]string{
						grovecorev1alpha1.LabelPodGang:   "test-podgang",
						grovecorev1alpha1.LabelPartOfKey: "missing-pgs",
					},
					OwnerReferences: []metav1.OwnerReference{
						{
							APIVersion: "grove.io/v1alpha1",
		...
		// Successfully prepares sync context even when PodGang doesn't exist yet
		{
			...
			pclq: &grovecorev1alpha1.PodClique{
				ObjectMeta: metav1.ObjectMeta{
					...
					Namespace: "default",
					Labels: map[string]string{
						grovecorev1alpha1.LabelPodGang:   "missing-podgang",
						grovecorev1alpha1.LabelPartOfKey: "test-pgs",
					},
					OwnerReferences: []metav1.OwnerReference{
						{
	...
	return &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				...
				grovecorev1alpha1.LabelPodClique:    pclqName,
			},
			OwnerReferences: []metav1.OwnerReference{
				{
					APIVersion: "grove.io/v1alpha1",
