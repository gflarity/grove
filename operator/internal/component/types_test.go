...
// TestComponentNameConstants validates that all component name constants have the expected values.
// These constants are used as labels on Kubernetes resources to identify components managed by the operator.
func TestComponentNameConstants(t *testing.T) {
	tests := []struct {
		// name describes the specific component constant being tested
		name string
		// actualValue is the constant value being validated
		actualValue string
		// expectedValue is what the constant should equal
		...
		{
			// Validates the component name for PodGang headless services
			name:          "NamePodGangHeadlessService",
			actualValue:   NamePodGangHeadlessService,
			expectedValue: "pgs-headless-service",
		},
		{
			// Validates the component name for Pod roles
			name:          "NamePodRole",
			actualValue:   NamePodRole,
			expectedValue: "pod-role",
		},
		{
			// Validates the component name for Pod role bindings
			name:          "NamePodRoleBinding",
			actualValue:   NamePodRoleBinding,
			expectedValue: "pod-role-binding",
		},
		{
			// Validates the component name for Pod service accounts
			name:          "NamePodServiceAccount",
			actualValue:   NamePodServiceAccount,
			expectedValue: "pod-service-account",
		},
		{
			// Validates the component name for service account token secrets
			name:          "NameServiceAccountTokenSecret",
			actualValue:   NameServiceAccountTokenSecret,
			expectedValue: "pod-sa-token-secret",
		},
		{
			// Validates the component name for PodCliqueScalingGroup resources
			name:          "NamePodCliqueScalingGroup",
			actualValue:   NamePodCliqueScalingGroup,
			expectedValue: "pgs-pod-clique-scaling-group",
		},
		{
			// Validates the component name for HorizontalPodAutoscaler resources
			name:          "NameHorizontalPodAutoscaler",
			actualValue:   NameHorizontalPodAutoscaler,
			expectedValue: "pgs-hpa",
		},
		{
			// Validates the component name for PodGang resources
			name:          "NamePodGang",
			actualValue:   NamePodGang,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.expectedValue, tt.actualValue, "component name constant should have expected value")
		})
	}
}
