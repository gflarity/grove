...
func TestNewPodCliqueBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			expectedName := grovecorev1alpha1.GeneratePodCliqueName(
				grovecorev1alpha1.ResourceNameReplica{Name: tt.pgsName, Replica: int(tt.pgsReplicaIndex)},
				tt.pclqTemplateName,
			)
			...
			assert.Equal(t, grovecorev1alpha1.PodGangSetKind, ownerRef.Kind)
			...
			expectedLabels := map[string]string{
				...
				grovecorev1alpha1.LabelAppNameKey:             expectedName,
				grovecorev1alpha1.LabelComponentKey:           grovecorev1alpha1.LabelComponentPGSPodCliqueValue,
				grovecorev1alpha1.LabelPodGangSetReplicaIndex: strconv.Itoa(int(tt.pgsReplicaIndex)),
			}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			expectedLabels := map[string]string{
				grovecorev1alpha1.LabelManagedByKey:                      grovecorev1alpha1.LabelManagedByValue,
				grovecorev1alpha1.LabelPartOfKey:                         tt.pgsName,
				grovecorev1alpha1.LabelPodCliqueScalingGroup:             tt.pcsgName,
				grovecorev1alpha1.LabelComponentKey:                      grovecorev1alpha1.LabelComponentPCSGPodCliqueValue,
				grovecorev1alpha1.LabelPodGangSetReplicaIndex:            strconv.Itoa(tt.pgsReplicaIndex),
				grovecorev1alpha1.LabelPodCliqueScalingGroupReplicaIndex: strconv.Itoa(tt.pcsgReplicaIndex),
			}
			for key, expectedValue := range expectedLabels {
				assert.Equal(t, expectedValue, builder.pclq.Labels[key], "Label %s should match", key)
			}
...
func TestPodCliqueBuilder_WithLabels(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			customLabels: map[string]string{
				grovecorev1alpha1.LabelComponentKey: "custom-component",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, templateName := range tt.pclqTemplateNames {
				expectedName := grovecorev1alpha1.GeneratePodCliqueName(
					grovecorev1alpha1.ResourceNameReplica{Name: pgsName, Replica: int(replicaIndex)},
					templateName,
				)
...
func TestCreateDefaultPodCliqueWithoutPodSpec(t *testing.T) {
	...
	// Verify basic metadata
	expectedName := grovecorev1alpha1.GeneratePodCliqueName(
		grovecorev1alpha1.ResourceNameReplica{Name: pgsName, Replica: int(pgsReplicaIndex)},
		pclqTemplateName,
	)
	...
	assert.Equal(t, grovecorev1alpha1.LabelManagedByValue, pclq.Labels[grovecorev1alpha1.LabelManagedByKey])
	assert.Equal(t, pgsName, pclq.Labels[grovecorev1alpha1.LabelPartOfKey])
	assert.Equal(t, expectedName, pclq.Labels[grovecorev1alpha1.LabelAppNameKey])
	assert.Equal(t, strconv.Itoa(int(pgsReplicaIndex)), pclq.Labels[grovecorev1alpha1.LabelPodGangSetReplicaIndex])
}
