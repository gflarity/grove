...
func TestWithPCSGMinAvailableBreached(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedConditionReason:   grovecorev1alpha1.ConditionReasonInsufficientAvailablePCSGReplicas,
		},
		{
...
func TestWithPCLQAvailable(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			minAvailableCondition := findCondition(pclq.Status.Conditions, grovecorev1alpha1.ConditionTypeMinAvailableBreached)
			...
			scheduledCondition := findCondition(pclq.Status.Conditions, grovecorev1alpha1.ConditionTypePodCliqueScheduled)
