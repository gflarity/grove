...
// resolveTopologyValues is the shared core for topology value resolution.
// It iterates cached pods, applies the filter predicate, and collects unique
// node label values for the given domain.
func resolveTopologyValues(
	domain string,
	topoInfo *TopologyInfo,
	cachedPods map[string]CachedPodInfo,
	cachedNodeLabels map[string]map[string]string,
	filter func(CachedPodInfo) bool,
) string {
...
func ResolveTopologyValue(
	...
	return resolveTopologyValues(domain, topoInfo, cachedPods, cachedNodeLabels,
		func(pod CachedPodInfo) bool {
...
func ResolveTopologyValueByReplicaIndex(
	...
	return resolveTopologyValues(domain, topoInfo, cachedPods, cachedNodeLabels,
		func(pod CachedPodInfo) bool {
			return pod.Labels[LabelPartOf] == pcsName &&
				pod.Labels[LabelPCSReplicaIndex] == replicaIndex
		})
}
